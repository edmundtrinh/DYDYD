...
import {
  ...
  MINIMUM_QUEST_DURATION_MINUTES,
} from '../constants';
...
describe('Compassionate Streak Constants', () => {
  ...
  it('should export MINIMUM_QUEST_DURATION_MINUTES as 2', () => {
    expect(MINIMUM_QUEST_DURATION_MINUTES).toBe(2);
  });
});
...
describe('shouldOfferComebackQuest', () => {
  ...
  it.each([
    ...
    [7, true, '7 days ago (boundary end)'],
    [8, false, '8 days ago (beyond max)'],
...
describe('getOnboardingQuestLimit', () => {
  it.each([
    [0, 1, 'initial (0 active days)'],
    [1, 1, '1 active day (not yet unlocked)'],
    [2, 1, '2 active days (not yet unlocked)'],
    [3, 3, '3 active days (first unlock: 1 + 2)'],
    [5, 3, '5 active days (between unlocks)'],
    [6, 5, '6 active days (second unlock: 1 + 4)'],
    [9, 7, '9 active days (third unlock: 1 + 6)'],
    [30, 21, '30 active days (10 unlocks: 1 + 20)'],
  ])(
