...
const healthSyncSchema = z.object({
  metrics: z.array(z.object({
    type: z.enum(['steps', 'distance', 'active_calories', 'sleep_hours', 'water_cups', 'workout_minutes', 'heart_rate', 'mindful_minutes', 'stand_hours']),
    value: z.number(),
    source: z.enum(['apple_health', 'google_fit', 'garmin', 'samsung_health', 'manual']),
    timestamp: z.string().refine((s) => !isNaN(Date.parse(s)), { message: 'Timestamp must be ISO 8601' }),
  })).min(1),
});
...
app.post(
  '/sync',
  authenticate,
  validateBody(healthSyncSchema),
  async (c) => {
    const { metrics } = c.get('validatedBody') as z.infer<typeof healthSyncSchema>;
    ...
    const result: HealthSyncResult = {
      ...
      dataPoints: metrics.map((m) => ({
        type: m.type as HealthDataType,
        value: m.value,
        unit: '',
        source: m.source as HealthDataSource,
